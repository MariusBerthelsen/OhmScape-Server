...
public class ClanChatManager {
	...
	public static void init() {
		try {
			if (new File(FILE_DIRECTORY).listFiles() == null) {
				System.out.println("Missing OhmScape.json in ClanChat directory. Exiting.");
				System.exit(0);
			}
			for (File file : (new File(FILE_DIRECTORY)).listFiles()) {
				...
				if (file.getName().equalsIgnoreCase("OhmScape.json")) {
					file.setReadable(true);
					file.setReadOnly();
					System.out.println("Set \"OhmScape.json\" to Read Only. (Anti-corruption)");
				}
	...
	public static void writeFile(ClanChat clan) {
		try {
			if (clan.getName().equalsIgnoreCase("OhmScape")) {
				System.out.println("Skipped writing OhmScape.json file because it's read only.");
				return;
			}
